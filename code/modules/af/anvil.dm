...
/obj/machinery/anvil/attackby(obj/item/reforge_target, mob/user, params)
	if(!reforge_target.GetComponent(/datum/component/fantasy))
		reforge_target.AddComponent(/datum/component/fantasy, 0, null, FALSE, FALSE, TRUE)
		// to_chat(user, span_danger("R"))
		// return
...
/obj/machinery/anvil/proc/reroll(obj/item/reforge_target, mob/user, datum/reforge_menu/player_menu)
	if(player_menu.use_super_credits)
		if(user.mind.super_credits < SUPER_CREDIT_REROLL_COST)
			to_chat(user, "You don't have any Super Monkey Credits(TM) to use!")
			player_menu.use_super_credits = FALSE
			return
		to_chat(user, "You spend a Super Monkey Credit(TM) to boost your reroll chances!")
	...
	reforge_target.AddComponent(/datum/component/fantasy, 0, null, FALSE, FALSE, TRUE) //this is bad code
	...
	set_light(l_outer_range = 3, l_inner_range = 3, l_power = 3, l_color = GLOB.rarity_to_color[new_rarity])
